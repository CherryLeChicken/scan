document.addEventListener('DOMContentLoaded', function () {
    ...
    function suggestCanadianAlternatives(productName) {
        ...
        fetch(`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${searchQuery}&search_simple=1&json=1&countries=canada`)
            .then(response => response.json())
            .then(data => {
                if (data.products && data.products.length > 0) {
                    let alternativesHTML = '<p>Canadian alternatives:</p><ul>';
                    data.products.forEach(product => {
                        alternativesHTML += `<li>${product.product_name}</li>`;
                    });
                    alternativesHTML += '</ul>';
                    infoDiv.innerHTML += alternativesHTML;
                } else {
